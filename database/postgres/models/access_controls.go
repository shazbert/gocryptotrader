...
var (
	...
	accessControlColumnsWithoutDefault = []string{"level", "name", "created_at", "updated_at"}
	accessControlColumnsWithDefault    = []string{"id"}
...
func FindAccessControl(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*AccessControl, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"access_controls\" where \"id\"=$1", sel,
	)
...
func (o *AccessControl) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO \"access_controls\" %sDEFAULT VALUES%s"
		}
...
func (o *AccessControl) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("models: no access_controls provided for upsert")
	}
	if !boil.TimestampsAreSkipped(ctx) {
		currTime := time.Now().In(boil.GetLocation())
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("models: unable to upsert access_controls, could not build update column list")
		}
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"access_controls\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert access_controls")
	}
...
func (o *AccessControl) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"access_controls\" WHERE \"id\"=$1"
...
func AccessControlExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"access_controls\" where \"id\"=$1 limit 1)"
