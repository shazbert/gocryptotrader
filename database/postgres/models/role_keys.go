...
var (
	...
	roleKeyColumnsWithoutDefault = []string{"key_id", "role_id", "created_at", "updated_at"}
	roleKeyColumnsWithDefault    = []string{"id"}
...
func (o *RoleKey) SetRole(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Role) error {
	...
	updateQuery := fmt.Sprintf(
		...
		strmangle.WhereClause("\"", "\"", 2, roleKeyPrimaryKeyColumns),
	)
...
func FindRoleKey(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*RoleKey, error) {
	roleKeyObj := &RoleKey{}
	...
	query := fmt.Sprintf(
		"select %s from \"role_keys\" where \"id\"=$1", sel,
	)
...
func (o *RoleKey) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO \"role_keys\" %sDEFAULT VALUES%s"
		}
...
func (o *RoleKey) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"role_keys\" SET %s WHERE %s",
			...
			strmangle.WhereClause("\"", "\"", len(wl)+1, roleKeyPrimaryKeyColumns),
		)
...
func (o RoleKeySlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"role_keys\" SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, roleKeyPrimaryKeyColumns, len(o)))
...
func (o *RoleKey) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("models: no role_keys provided for upsert")
	}
	if !boil.TimestampsAreSkipped(ctx) {
		currTime := time.Now().In(boil.GetLocation())
	...
	roleKeyUpsertCacheMut.RLock()
	cache, cached := roleKeyUpsertCache[key]
	roleKeyUpsertCacheMut.RUnlock()
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("models: unable to upsert role_keys, could not build update column list")
		}

		conflict := conflictColumns
		if len(conflict) == 0 {
			conflict = make([]string, len(roleKeyPrimaryKeyColumns))
			copy(conflict, roleKeyPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"role_keys\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(roleKeyType, roleKeyMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
			cache.retMapping, err = queries.BindMapping(roleKeyType, roleKeyMapping, ret)
			if err != nil {
				return err
			}
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert role_keys")
	}

	if !cached {
		roleKeyUpsertCacheMut.Lock()
		roleKeyUpsertCache[key] = cache
		roleKeyUpsertCacheMut.Unlock()
	}
...
func (o *RoleKey) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"role_keys\" WHERE \"id\"=$1"
...
func (o RoleKeySlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"role_keys\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, roleKeyPrimaryKeyColumns, len(o))
...
func RoleKeyExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"role_keys\" where \"id\"=$1 limit 1)"
