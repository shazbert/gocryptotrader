...
func TestInsertDeleteTradeHistoryData(t *testing.T) {
	...
	fullHistory, err := db.GetFullPlatformHistory("Bitstamp", "BTCUSD", "SPOT")
	if err != nil {
		t.Error("test failed - GetFullPlatformHistory() error", err)
	}

	if len(fullHistory) != 1 {
		t.Error("test failed - GetFullPlatformHistory() error unexpected values")
	}
