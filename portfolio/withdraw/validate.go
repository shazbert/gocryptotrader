...
func (r *Request) validateCrypto() (err []string) {
	if !portfolio.IsWhiteListed(r.Crypto.Address) {
		err = append(err, ErrStrAddressNotWhiteListed)
	}

	if !portfolio.IsExchangeSupported(r.Exchange, r.Crypto.Address) {
		err = append(err, ErrStrExchangeNotSupportedByAddress)
	}

	if portfolio.IsTagOrMemoRequired(r.Crypto.Address) &&
		r.Crypto.AddressTag == "" {
