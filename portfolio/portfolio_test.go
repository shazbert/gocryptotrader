...
func TestGetEthereumBalance(t *testing.T) {
	...
	_, err = GetEthereumBalance(nonsenseAddress)
	if !errors.Is(err, errNotEthAddress) {
		t.Fatalf("expected: %v but received: %v", errNotEthAddress, err)
	}
}
...
func TestGetCryptoIDBalance(t *testing.T) {
	...
	if !errors.Is(err, errInvalidAddress) {
		t.Fatalf("expected: %v but received: %v", errInvalidAddress, err)
	}
...
func TestGetXRPBalance(t *testing.T) {
	xrpAddress := "bruh"
	_, err := GetRippleBalance(xrpAddress)
	if !errors.Is(err, errNoBalanceReturned) {
		t.Fatalf("expected: %v but received: %v", errNoBalanceReturned, err)
	}
...
func TestGetAddressBalance(t *testing.T) {
	...
	portfolio := Base{}
	err := portfolio.Seed(State{})
	if err != nil {
		t.Fatal(err)
	}

	err = portfolio.UpdateInsertColdWallet(&Wallet{
		Address:            ltcAddress,
		WhiteListed:        false,
		TagMemoRequired:    true,
		SupportedExchanges: "ALL",
		Holding: Holding{
			Currency: ltc.String(),
