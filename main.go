...
func main() {
	...
	flag.BoolVar(&settings.EnableRecentTradeSyncing, "tradesync", true, "enables trade syncing for all enabled exchanges")
	flag.BoolVar(&settings.EnableContinuousSync, "continuousSync", true, "enables the sync manager to continuously sync")
	flag.IntVar(&settings.SyncWorkerCount, "syncworkers", 15, "specifies the total sync manager worker amount")
	flag.BoolVar(&settings.EnableHistoricTradeSyncing, "historicTradeSync", false, "enables syncing of historic trades for all enabled exchanges")
