...
func printPlatformTradeSummary(t *[]exchange.PlatformTrade, exchangeName, assetType string, p pair.CurrencyPair, timestampStart, timestampEnd time.Time, err error) {
	if err != nil {
...
func fetchHistory(exch exchange.IBotExchange, p pair.CurrencyPair, assetType string, timeStart, timeEnd time.Time) {
	if timeEnd.IsZero() {
		timeEnd = time.Now()
	}
