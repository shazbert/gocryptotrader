// This tool will initiaite a authenticated request generating an initial nonce
// across all supported exchanges and then sleep for 1 minute which will allow
// us to determine if there is any timestamp issues.
package main
...
func main() {
	...
	for i := range SupportedExchanges {
		...
		if !cfg.Enabled {
			cfg.Report = &Report{errors.New("not enabled in timestamp.json")}
			continue
		}

		if cfg.Keys == (Keys{}) {
			cfg.Report = &Report{errors.New("API Keys not set in timestamp.json")}
			continue
		}
	...
	select {
	case <-interupted:
		log.Println("Interuption caught shutting down")
	case <-finished:
	}
	log.Println()
	log.Println("Report:")
...
func LoadConfig() bool {
	...
	if err != nil {
		log.Println("Config file not found, creating timestamp.json configuration.")
		log.Println("Please set exchange `API Keys` and set to `enabled` to verify authenticated endpoints.")
