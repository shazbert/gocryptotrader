...
// intial operations
func (b *BTCMarkets) GetPlatformHistory(p pair.CurrencyPair, assetType string, timestampStart time.Time, tradeID string) ([]exchange.PlatformTrade, error) {
	...
	t, err := b.GetTrades(p.FirstCurrency.String(),
		p.SecondCurrency.String(),
		v)
	if err != nil {
		return resp, err
