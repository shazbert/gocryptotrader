...
// intial operations
func (g *Gemini) GetPlatformHistory(p pair.CurrencyPair, assetType string, timestampStart time.Time, tradeID string) ([]exchange.PlatformTrade, error) {
	...
	trades, err := g.GetTrades(p.Pair().String(), v)
	if err != nil {
		return resp, err
	}
