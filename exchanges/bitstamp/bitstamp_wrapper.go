...
func (b *Bitstamp) GetExchangeHistory(p *exchange.TradeHistoryRequest) ([]exchange.TradeHistory, error) {
	var resp []exchange.TradeHistory
	values := url.Values{}
	values.Set("time", "minute")

	trans, err := b.GetTransactions(b.FormatExchangeCurrency(p.Pair, p.Asset).String(),
		values)
	if err != nil {
		return resp, err
	}
