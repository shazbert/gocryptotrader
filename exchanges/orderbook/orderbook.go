...
func SubscribeToExchangeOrderbooks(exchange string) (dispatch.Pipe, error) {
	...
	if !ok {
		return dispatch.Pipe{}, fmt.Errorf("%w for %s exchange", ErrCannotFindOrderbook, exchange)
	}
...
func (s *Service) Retrieve(exchange string, p currency.Pair, a asset.Item) (*Base, error) {
	...
	m1, ok := s.books[strings.ToLower(exchange)]
	if !ok {
		return nil, fmt.Errorf("%w for %s %s %s", ErrCannotFindOrderbook, exchange, p, a)
	}
	book, ok := m1.m[a][p.Base.Item][p.Quote.Item]
	if !ok {
		return nil, fmt.Errorf("%w for %s %s %s", ErrCannotFindOrderbook, exchange, p, a)
	}
