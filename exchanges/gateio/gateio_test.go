...
func TestWsTickerPushData(t *testing.T) {
	t.Parallel()
	got, err := g.WsHandleSpotData(context.Background(), []byte(wsTickerPushDataJSON))
	require.NoError(t, err)
	require.NotNil(t, got)
	a, ok := got.(ticker.Prices)
	require.True(t, ok)
	require.Len(t, a, 3)
	require.Equal(t, float64(19106.55), a[0].Last)
	require.Equal(t, float64(19108.71), a[0].Ask)
	require.Equal(t, float64(19106.55), a[0].Bid)
	require.Equal(t, float64(2811.3042155865), a[0].Volume)
	require.Equal(t, float64(53441606.52411221454674732293), a[0].QuoteVolume)
	require.Equal(t, float64(19417.74), a[0].High)
	require.Equal(t, float64(18434.21), a[0].Low)
	require.Equal(t, "BTC_USDT", a[0].Pair.String())
}
