...
func (p *Poloniex) wsHandleData(respRaw []byte) error {
	...
	case wsAccountNotificationID:
		...
		for i := range notificationsArray {
			fmt.Println(notificationsArray[i])
...
func (p *Poloniex) processAccountPendingOrder(notification []interface{}) error {
	...
	// THIS IS A FLOAT64 TESTED!!!
	side, ok := notification[5].(float64)
	if !ok {
		return fmt.Errorf("%w order type not float64", errTypeAssertionFailure)
	}
	orderSide := order.Buy
	if side == 0 {
...
func (p *Poloniex) processAccountBalanceUpdate(notification []interface{}) error {
	...
	accountType, ok := notification[2].(string)
	if !ok {
	...
	return p.Holdings.AdjustByBalance(deriveAccountType(accountType),
		asset.Spot,
		code,
		amount)
}

func deriveAccountType(s string) string {
	switch s {
	case "e":
		return "exchange"
	case "m":
