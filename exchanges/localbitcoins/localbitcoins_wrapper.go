...
// intial operations
func (l *LocalBitcoins) GetPlatformHistory(p pair.CurrencyPair, assetType string, timestampStart time.Time, tradeID string) ([]exchange.PlatformTrade, error) {
	...
	t, err := l.GetTrades(p.SecondCurrency.Lower().String(), v)
	if err != nil {
		return resp, err
	}
