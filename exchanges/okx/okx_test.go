...
// 6653702	       180.2 ns/op	       4 B/op	       1 allocs/op
func BenchmarkGetAssetsFromInstrumentType(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, err := ok.GetAssetsFromInstrumentTypeOrID(okxInstTypeOption, "")
		require.NoError(b, err)
	}
}

//	2696780	       503.6 ns/op	     294 B/op	      21 allocs/op (current)
//
// 1580508	       971.9 ns/op	     509 B/op	      20 allocs/op
func BenchmarkGetAssetsFromID(b *testing.B) {
	go func() {
		// Test lock impedance
		for {
			_, _ = ok.GetAssetsFromInstrumentTypeOrID("", "BTC-USDT")
		}
	}()

	// Test worse case path
	pair := ok.CurrencyPairs.Pairs[asset.Futures].Enabled[0].String()

	b.ResetTimer()
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		_, _ = ok.GetAssetsFromInstrumentTypeOrID("", pair)
	}
}
