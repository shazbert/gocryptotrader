...
func TestGetHolding(t *testing.T) {
	h, err := DeployHoldings("getHolding")
	...
	values := map[*currency.Item]Value{
		currency.BTC.Item: {Total: 1},
		currency.LTC.Item: {Total: 20},
	}
...
func TestLoad(t *testing.T) {
	...
	values = map[*currency.Item]Value{
		currency.BTC.Item: {Total: 2, Locked: 0.5},
		currency.XRP.Item: {Total: 60000},
	}
...
func TestAdjustHolding(t *testing.T) {
	h, err := DeployHoldings("adjustholdings")
	if !errors.Is(err, nil) {
		t.Fatalf("expected: %v but received: %v", nil, err)
	}
	...
	values := map[*currency.Item]Value{
		currency.XRP.Item: {Total: 40.5, Locked: 6},
	}
