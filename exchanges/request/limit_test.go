...
func TestRateLimit(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) { //nolint:thelper,nolintlint // false positive
		err := (*RateLimiterWithWeight)(nil).RateLimit(t.Context(), 0)
		...
		start = time.Now()
		err = r.RateLimit(WithDelayNotAllowed(t.Context()), 0)
		synctest.Wait()
		elapsed = time.Since(start)
		...
		require.NoError(t, cancelledLimiter.RateLimit(t.Context(), 0), "first reservation must not error")
		ctx, cancel = context.WithCancel(t.Context())
		cancel()
		require.ErrorIs(t, cancelledLimiter.RateLimit(ctx, 0), context.Canceled, "cancelled wait must return context cancellation")
		start = time.Now()
		require.NoError(t, cancelledLimiter.RateLimit(t.Context(), 0), "reservation after cancellation must not error")
		...
		require.NoError(t, r.RateLimit(t.Context(), 0), "released reservation must be immediately reusable")
...
func TestRateLimitMultiple(t *testing.T) {
	t.Parallel()
	...
	synctest.Test(t, func(t *testing.T) { //nolint:thelper,nolintlint // false positive
		...
		require.NoError(t, endpoint.RateLimit(t.Context(), 0, additionalRateLimits...), "first reservation must not error")

		start := time.Now()
		err := endpoint.RateLimit(t.Context(), 0, additionalRateLimits...)
		elapsed := time.Since(start)
		...
		err = endpoint.RateLimit(t.Context(), 0, RateLimitWithWeightOverride{Limiter: &RateLimiterWithWeight{limiter: rate.NewLimiter(rate.Limit(1), 1)}})
		assert.ErrorIs(t, err, errInvalidWeight, "zero weight should return errInvalidWeight")

		err = endpoint.RateLimit(t.Context(), 0, RateLimitWithWeightOverride{Limiter: nil, WeightOverride: 1})
		assert.ErrorContains(t, err, "nil pointer: *request.RateLimiterWithWeight")
	})
}
...
func TestRateLimitAll_WithDelayNotAllowed(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) { //nolint:thelper,nolintlint // false positive
		...
		err := endpoint.RateLimit(WithDelayNotAllowed(t.Context()), 0, additionalRateLimits...)
...
func TestRateLimitWithAdditional(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) { //nolint:thelper,nolintlint // false positive
		...
		err = endpoint.RateLimit(t.Context(), 0, RateLimitWithWeightOverride{WeightOverride: 1})
...
func TestRateLimitWithWeightOverride(t *testing.T) {
	t.Parallel()
	...
	synctest.Test(t, func(t *testing.T) { //nolint:thelper,nolintlint // false positive
		...
		start := time.Now()
		err := weightedEndpoint.RateLimit(t.Context(), 3, RateLimitWithWeightOverride{Limiter: weightedExtra, WeightOverride: 1})
		elapsed := time.Since(start)
		...
		require.NoError(t, standaloneEndpoint.RateLimit(t.Context(), 5), "first weighted endpoint reservation must not error")
		start = time.Now()
		err = standaloneEndpoint.RateLimit(t.Context(), 5)
		elapsed = time.Since(start)
...
func TestRateLimit_Concurrent_WithFailure(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) { //nolint:thelper,nolintlint // false positive
		...
		for i := range 10 {
			...
			errs.Go(func() error { return r.RateLimit(ctx, 0) })
		}
...
func TestRateLimit_Concurrent(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) { //nolint:thelper,nolintlint // false positive
		...
		errs := common.ErrorCollector{}
		for range 10 {
			errs.Go(func() error { return r.RateLimit(t.Context(), 0) })
		}
...
func TestRateLimit_Linear_WithFailure(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) { //nolint:thelper,nolintlint // false positive
		...
		for i := range 10 {
			...
			if err := r.RateLimit(ctx, 0); err != nil {
...
func TestRateLimit_Linear(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) { //nolint:thelper,nolintlint // false positive
		...
		tn := time.Now()
		for range 10 {
			require.NoError(t, r.RateLimit(t.Context(), 0))
		}
		assert.Less(t, time.Since(tn), time.Second, "should complete within reasonable time")
	})
}
